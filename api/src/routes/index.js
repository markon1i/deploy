...
router.get("/dogs", async (req, res) => {
  try {
    ...
    if (name) {
      const raza = (await axios.get(`${DB_API}/search?q=${name}`)).data;
      //console.log(raza);
      if (raza.length === 0)
        return res.status(404).send("no se econtro raza de perro");
      else {
      ...
      const dogsApi = (await axios.get(`${DB_API}`)).data;
      //console.log(dogsApi);
      const dogs = await Promise.all(
        dogsApi.map(async (dog) => {
          const nameTemperament = dog.temperament?.split(", ");
          console.log(nameTemperament);
          if (dog !== undefined) {
            const temperament = (
              await Promise.all(
                nameTemperament !== undefined
                  ? nameTemperament.map(async (name) => {
...
router.get("/dogs/:id", async (req, res) => {
  ...
  if (Number(id)) {
    try {
      axios.get(`${DB_API}`).then((response) => {
...
router.get("/temperaments", async (req, res) => {
  try {
    const temperamentApi = (await axios.get(`${DB_API}`)).data;
